...
@SpringBootTest
class SynchronizedPointServiceTest {
    ...
    @BeforeEach
    void setUp() {
        synchronizedPointService.init(userId, 0L);
    }
    ...
    @Test
    @DisplayName("synchronized - 10개 스레드가 동시에 1000원씩 충전하면 최종 잔액은 10000원이다")
    void concurrentCharge() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try {
                    synchronizedPointService.charge(userId, chargeAmount);
                } finally {
                    latch.countDown();
                }
            });
        ...
        Point point = synchronizedPointService.getPoint(userId);
