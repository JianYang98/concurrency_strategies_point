...
@SpringBootTest
class ReentrantLockPointServiceTest {
    ...
    @BeforeEach
    void setUp() {
        reentrantLockPointService.init(userId, 0L);
    }
    ...
    @Test
    @DisplayName("ReentrantLock - 10개 스레드가 동시에 1000원씩 충전하면 최종 잔액은 10000원이다")
    void concurrentCharge() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try {
                    reentrantLockPointService.charge(userId, chargeAmount);
                } finally {
                    latch.countDown();
                }
            });
        ...
        Point point = reentrantLockPointService.getPoint(userId);
